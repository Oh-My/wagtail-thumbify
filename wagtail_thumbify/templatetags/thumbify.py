...
class ThumbifyImageNode(template.Node):
    ...
    def render(self, context):
        ...
        if self.output_var_name:
            ...
            return mark_safe('<img src="{}"{}>'.format(processed_image_url, flatatt(resolved_attrs)))
    ...
    def process_image(self, image, width = None, height = None, fit_in = False):
        ...
        if type(image) == WagtailImage:
            original_image_path = str(Path(settings.MEDIA_ROOT).joinpath(image.file.name))
        ...
        resized_dir = Path(settings.MEDIA_ROOT).joinpath('resized').joinpath(src_hash[0:2])
        ...
        with NamedTemporaryFile(mode='wb+') as tmp_image:
            if type(image) == WagtailImage:
                ...
                if not re.match(r'^https?://', image):
                    raise template.TemplateSyntaxError('Image parameter should be either an image object or an absolute URL')
            ...
            if fit_in:
                ...
                box = {
                    'left': round((pil_image.width - crop_width) / 2),
                    'upper': round((pil_image.height - crop_height) / 2),
                    'right': round((pil_image.width - crop_width) / 2) + crop_width,
                    'lower': round((pil_image.height - crop_height) / 2) + crop_height,
                }

                pil_image = pil_image.crop(box.values())
